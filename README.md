...
Successful chat events and verified autonomous Action outcomes create versioned first-person Experience records under the durable subject snapshot. `GET /api/experiences`, `GET /api/experiences/{experience_id}`, and evidence-backed `POST /api/experiences/{experience_id}/revisions` are admin-only. Experience records separate external observation references from internal structured interpretation and appraisal; they do not duplicate chat text, generated responses, prompts, attachments, or hidden thoughts. Their subjective salience influences Working Memory retention, Attention, episodic retrieval ordering, and consolidation eligibility through the nonzero typed `memory.consolidation_min_arousal` and `memory.consolidation_min_subjective_salience` thresholds. Reassessment accepts verified Observation or reviewed Feedback refs and appends revisions instead of rewriting history.
