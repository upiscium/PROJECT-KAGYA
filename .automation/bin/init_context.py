...
def default_branch(root: Path) -> str:
    ...
    result = run(
        ["gh", "repo", "view", "--json", "defaultBranchRef"], cwd=root, check=False
    )
...
def task_state_is_ignored(root: Path) -> bool:
    result = run(
        ["git", "check-ignore", "-q", ".task-state/task.md"], cwd=root, check=False
    )
    if result.returncode == 0:
        return True
...
def validate_identity(
    root: Path, branch: str, base: str, state: dict | None
) -> str | None:
    if branch == base:
    ...
    valid_branch = branch.startswith(f"task/{task_id}-") or branch.startswith(
        f"fix/{task_id}-"
    )
    if not valid_branch:
        raise InitError(
            f"branch/Task identity mismatch: branch={branch}, task={task_id}"
        )
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="Read-only Agent initialization checks"
    )
