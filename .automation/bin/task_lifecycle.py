...
def current_worktree(root: Path) -> WorktreeRecord:
    matches = [
        record for record in parse_worktrees(root) if record.path == root.resolve()
    ]
    if len(matches) != 1:
        raise LifecycleError(
...
def worktree_for_task(root: Path, task: str) -> WorktreeRecord:
    ...
    candidates = [
        ...
        if branch_matches_task(record.branch, task)
    ]
    if len(candidates) != 1:
        raise LifecycleError(
...
def task_cleanup(root: Path, task: str) -> None:
    ...
    if ahead:
        raise LifecycleError(
            f"cleanup refused: Task branch has {ahead} unpushed commit(s)"
        )
    ...
    if status == "merged":
        if pr_result.returncode != 0:
            raise LifecycleError(
                "cleanup refused: merged Task has no resolvable pull request"
            )
        ...
        if data.get("state") == "OPEN":
            raise LifecycleError(
                "cleanup refused: cancelled Task still has an open pull request"
            )
...
def parser() -> argparse.ArgumentParser:
    ...
    packed_batch = sub.add_parser("batch-plan-packed")
    packed_batch.add_argument("tasks")
    return result
...
def main() -> int:
    ...
    try:
        ...
        elif args.command == "batch-plan-packed":
            batch_plan(root, args.tasks.split())
