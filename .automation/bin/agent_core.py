...
BASE_BRANCH_RE = re.compile(r"(?m)^- Base branch: (.+)$")
...
def task_state_identity(root: Path) -> dict[str, str]:
    state_path = task_state_path(root)
    try:
        state = state_path.read_text(encoding="utf-8")
    except FileNotFoundError as exc:
        raise AutomationError("Task operation requires Task State") from exc
    identity: dict[str, str] = {}
    for key, pattern in (
        ("task", TASK_ID_RE),
        ("branch", BRANCH_RE),
        ("worktree", WORKTREE_RE),
        ("base", BASE_BRANCH_RE),
    ):
        match = pattern.search(state)
        if match is None:
            raise AutomationError(f"Task State missing identity field: {key}")
...
def ensure_task_branch(root: Path, task: str) -> str:
    ...
    if not (branch.startswith(f"task/{task}-") or branch.startswith(f"fix/{task}-")):
        raise AutomationError(
            f"current branch {branch!r} is not the Task branch for {task}"
        )
    if branch == default_branch(root):
        raise AutomationError("Task operation refused on the default branch")
    identity = task_state_identity(root)
    if identity["task"] != task or identity["branch"] != branch:
        raise AutomationError("Task State identity does not match the requested Task")
    if Path(identity["worktree"]).resolve() != root.resolve():
        raise AutomationError("Task State worktree does not match the current worktree")
    return branch


def publication_base(root: Path, task: str) -> str:
    identity = task_state_identity(root)
    if identity["task"] != task:
        raise AutomationError("Task State identity does not match the requested Task")
    base = identity["base"]
    allowed = policy(root).get("publication", {}).get("base_branches")
    if allowed is None:
        allowed = [default_branch(root)]
    if base not in allowed:
        raise AutomationError(
            f"Task base branch is not allowed for publication: {base}"
        )
    return base
...
def reject_unsafe_paths(
    root: Path, paths: list[str], *, allow_automation_core: bool = False
) -> None:
    cfg = policy(root)
    ...
    if bad_core and not allow_automation_core:
        raise AutomationError(
            "ordinary Task modifies Automation Core: " + ", ".join(bad_core)
        )
    lowered = [(path, path.lower()) for path in paths]
    bad_secret = [
        path
        for path, low in lowered
        if any(token.lower() in low for token in secret_names)
    ]
    if bad_secret:
        raise AutomationError(
            "potential secret file in Task changes: " + ", ".join(bad_secret)
        )
...
def task_start(root: Path, task: str, slug: str) -> None:
    ...
    base_revision = git(
        "rev-parse", "--verify", base_ref, cwd=root, check=False
    ) or git("rev-parse", base, cwd=root)
    ...
    run(
        ["git", "worktree", "add", "-b", branch, str(worktree), base_revision], cwd=root
    )
...
def doctor(root: Path) -> None:
    missing = [
        tool for tool in ("git", "gh", "just", "python3") if shutil.which(tool) is None
    ]
    if missing:
        ...
        json.dumps(
            {
                "task": task,
                "branch": branch,
                "status": git("status", "--short", cwd=root).splitlines(),
            }
        )
    )
...
def commit_task(root: Path, task: str, message: str) -> None:
    ...
    maintenance = os.environ.get("AUTOMATION_MAINTENANCE") == "1"
    reject_unsafe_paths(root, paths, allow_automation_core=maintenance)
    run(["git", "add", "--", *paths], cwd=root)
    run(["git", "diff", "--cached", "--check"], cwd=root)
    staged = git("diff", "--cached", "--name-only", cwd=root).splitlines()
    reject_unsafe_paths(root, staged, allow_automation_core=maintenance)
...
def pr_for_branch(root: Path, branch: str) -> dict | None:
    result = run(
        [
            ...
            branch,
            "--json",
            "number,title,body,headRefName,baseRefName,isDraft,state,headRefOid",
        ],
        cwd=root,
        check=False,
    )
...
def pr_create(root: Path, task: str) -> None:
    ...
    base = publication_base(root, task)
...
def cleanup(root: Path, task: str) -> None:
    ...
    print(
        "cleanup must be executed by the Main worktree in the lifecycle extension (#8)"
    )
...
def pr_details(root: Path, pr: str) -> dict:
    data = json.loads(
        gh(
            ...
            pr,
            "--json",
            "number,baseRefName,headRefName,headRefOid,isDraft,mergeable,statusCheckRollup,state",
            cwd=root,
        )
    )
    return data
...
def validate_integration(root: Path, pr: str) -> dict:
    ...
    if allowed is None:
        allowed = [default_branch(root)]
    if data["baseRefName"] not in allowed:
        raise AutomationError("PR base is not an allowed publication branch")
    if data["isDraft"]:
    ...
    for check in data.get("statusCheckRollup") or []:
        ...
        if status and status != "COMPLETED":
            failures.append(
                check.get("name") or check.get("context") or "pending check"
            )
    ...
    if failures:
        raise AutomationError(
            "required checks are not successful: " + ", ".join(failures)
        )
    return data
...
def integrate_check(root: Path, pr: str) -> None:
    ...
    integration_checkpoint(root, pr).write_text(
        data["headRefOid"] + "\n", encoding="utf-8"
    )
    print(
        json.dumps(
            {"pr": data["number"], "head": data["headRefOid"], "status": "verified"}
        )
    )
...
def integrate_merge(root: Path, pr: str) -> None:
    ...
    if data["headRefOid"] != expected:
        raise AutomationError(
            f"PR head moved after integration check: expected {expected}, got {data['headRefOid']}"
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    start = agent_cmd.add_parser("task-start")
    ...
    for name in (
        ...
        p = agent_cmd.add_parser(name)
        p.add_argument("task")
    commit = agent_cmd.add_parser("commit")
    ...
    for name in ("check", "merge"):
        p = integrate_cmd.add_parser(name)
        p.add_argument("pr")
    return parser
