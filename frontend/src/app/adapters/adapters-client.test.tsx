...
function adapter(overrides: Partial<Adapter> = {}): Adapter {
  return {
    adapter_id: "candidate", base_model: "base", path: "/private/candidate", status: "candidate", dataset_path: "dataset", dataset_hash: "hash", eval_score: null, eval_result_path: null, created_at: "", updated_at: "", notes: "", base_model_revision: "rev", adapter_hash: "raw-adapter-hash", parent_adapter_id: null, parent_adapter_hash: null, activation_sequence: null, dataset_repetition_count: 0, dataset_overlap_count: 0, dataset_overlap_ratio: 0, holdout_score: null, holdout_baseline_score: null, holdout_regression: false, drift_scores: null, quality_gate_passed: true, holdout_gate_passed: true, drift_gate_passed: true, activation_gate_passed: false, behavioral_evaluation_id: null, behavioral_evaluation_path: null, behavioral_result_hash: null, behavioral_gate_passed: null, behavioral_candidate_adapter_hash: null, behavioral_base_model_revision: null, subject_revision: null, fixture_set_hash: null, behavioral_artifact_state: "unbound", deterministic_behavioral_artifact_status: "not_run", real_model_behavioral_evaluation_id: null, real_model_behavioral_gate_passed: null, real_model_behavioral_artifact_state: "unbound", real_model_behavioral_artifact_status: "not_run", behavioral_artifact_hash_match: "not_run", activation_eligibility_reason: "behavioral_unevaluated", real_model_behavioral_required: false, legacy_activation_warning: false, rollout_state: "candidate", canary_failures: 0, rollback_target_id: null,
    ...overrides,
  };
}
...
describe("AdaptersClient", () => {
  ...
  it.each([
    ...
    renderWithQuery();
    expect(await screen.findByText(`Behavioral deterministic: ${gateText}`)).toBeInTheDocument();
    expect(screen.getByText(`Deterministic artifact: ${artifactText}`)).toBeInTheDocument();
  ...
  it("shows real-model status separately and preserves exact eligibility reason", async () => {
    mockAdapter(adapter({ real_model_behavioral_required: true, activation_eligibility_reason: "real_model_behavioral_unevaluated" }));
    renderWithQuery();
    expect(await screen.findByText("Real-model artifact: not run")).toBeInTheDocument();
  ...
  it("never renders an unbound synthetic claim as behavioral passed", async () => {
    mockAdapter(adapter({ behavioral_evaluation_id: "synthetic-contract", behavioral_gate_passed: true }));
    renderWithQuery();
    expect(await screen.findByText("Behavioral deterministic: not run")).toBeInTheDocument();
    expect(screen.queryByText("Behavioral deterministic: passed")).not.toBeInTheDocument();
  });
});
