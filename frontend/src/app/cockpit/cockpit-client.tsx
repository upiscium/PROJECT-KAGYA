...
export function CockpitClient() {
  ...
  const loadedReceipts = new Set(actions.data?.traces.flatMap((item) => item.receipt ? [item.receipt.receipt_id] : []));
    ...
    <div className="page cockpit-page">
      ...
      <div className="cockpit-grid">
        ...
        <Section title="Recent Decisions" query={decisions} empty={decisions.data?.decisions.length === 0} emptyText="No decisions recorded.">
          <div className="stack">{recentDecisions.map((decision) => <DecisionRecord key={decision.decision_id} decision={decision} actions={actions.data?.traces ?? []} outbox={outbox.data?.messages ?? []} loadedContexts={loadedContexts} loadedGoals={loadedGoals} loadedPlans={loadedPlans} loadedCommitments={loadedCommitments} loadedOutbox={loadedOutbox} loadedActions={loadedActions} />)}</div>
        </Section>

        <Section title="Action Execution" query={actions} empty={actions.data?.traces.length === 0} emptyText="No action execution traces.">
          ...
          <div className="stack">{actions.data?.traces.map((trace) => <ActionTraceRecord key={trace.intent_id} trace={trace} loadedDecisions={loadedDecisions} loadedPlans={loadedPlans} loadedJournal={loadedJournal} />)}</div>
        </Section>
...
function DecisionRecord({ decision, actions, outbox, loadedContexts, loadedGoals, loadedPlans, loadedCommitments, loadedOutbox, loadedActions }: { decision: Decision; actions: CockpitActionTrace[]; outbox: CockpitOutboxMessage[]; loadedContexts: Set<string>; loadedGoals: Set<string>; loadedPlans: Set<string>; loadedCommitments: Set<string>; loadedOutbox: Set<string>; loadedActions: Set<string> }) {
