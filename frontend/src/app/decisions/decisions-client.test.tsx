...
it("renders structured safe explanation revisions and renderer status", async () => {
  fetchMock.mockResolvedValue({ ok: true, json: async () => ({ explanations: [{
    ...
    contributions: [{ source_type: "value", source_id: "care", source_revision: 2, contribution: 0.4, availability: "available" }],
    evidence_refs: ["evidence-1"],
    uncertainty: [{ code: "selected_candidate_uncertainty", severity: 0.3, refs: ["candidate-1"] }], information_gap_codes: ["evidence_missing"],
    risk: { risk_class: "read_only", policy_status: "allowed", approval_status: "not_required" }, reason_codes: [],
    outcome: { status: "failed", utility: -0.3, prediction_error: -0.5 }, change: { previous_explanation_revision: 1, changed_fields: ["outcome"], reason_codes: [] },
    renderer: { state: "failed", deterministic_template: "decision_explanation.action_failed.resolved.failed.v1", visible_explanation: "disposition=action_failed", failure_code: "renderer_invalid_or_unavailable" },
  }] }) });
  ...
  expect(screen.getByText(/Renderer: renderer_invalid_or_unavailable/)).toBeInTheDocument();
