...
function DebugResult({ result }: { result: DebugChatResponse }) {
  return (
    <div className="grid">
      ...
      <Card><CardTitle>Structured Behavior</CardTitle><p><Badge>{result.behavior_class}</Badge></p><p>Parse: {result.response_parse_valid ? "valid" : "invalid"}</p><p className="muted">{result.response_status}</p></Card>
      <Card><CardTitle>Hidden Thought</CardTitle><pre>{result.hidden_thought || "none"}</pre></Card>
      <Card><CardTitle>Loss + Emotion</CardTitle><p>Loss {result.loss === null ? "invalid" : formatNumber(result.loss)}</p><p>Novelty {result.appraisal?.novelty == null ? "invalid" : formatNumber(result.appraisal.novelty)}</p><p>Goal progress {result.appraisal ? formatNumber(result.appraisal.goal_progress) : "unknown"}</p><p>Threat {result.appraisal ? formatNumber(result.appraisal.threat) : "unknown"}</p><p>Controllability {result.appraisal ? formatNumber(result.appraisal.controllability) : "unknown"}</p><p>Certainty {result.appraisal ? formatNumber(result.appraisal.certainty) : "unknown"}</p><p>Valence {formatNumber(result.emotion.valence)}</p><p>Arousal {formatNumber(result.emotion.arousal)}</p><p>Reasons: {result.emotion_update?.reasons.join(", ") ?? "unknown"}</p></Card>
