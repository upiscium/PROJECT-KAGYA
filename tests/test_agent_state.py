...
def test_snapshot_round_trip_restores_internal_state(tmp_path: Path) -> None:
    ...
    relationship = loop.relationship_store.ensure_interlocutor("person-1")
    ...
    assert restored_loop.persistent_state.active_goals[0]["schema_version"] == 3
    ...
    assert (
        restored_loop.relationship_store.for_interlocutor("person-1").relationship_id
