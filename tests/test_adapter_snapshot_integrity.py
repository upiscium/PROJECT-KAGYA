...
def test_concurrent_snapshots_for_same_hash_are_not_corrupted(
    ...
    assert len(set(loaded_paths)) == 1
    assert all(item.adapter_artifact_manifest_hash == manifest.sha256 for item in providers)
    assert build_adapter_artifact_manifest(loaded_paths[0]) == manifest
