...
def test_build_parser_has_expected_defaults() -> None:
    ...
    assert args.preset == "lightweight"
...
def test_resolve_model_name_prefers_override() -> None:
    assert cli.resolve_model_name("lightweight", "custom/model") == "custom/model"


def test_resolve_model_name_uses_preset() -> None:
    assert cli.resolve_model_name("qwen3.5-9b", None) == "Qwen/Qwen3.5-9B-Instruct"
